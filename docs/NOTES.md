...
**解投（2026-09-16 連戰資料）**：只有普通投能解。起手 CommandCode 363（T）20/20、400 28/28 都用 T 破掉；方向投抓住時的 cmd 364／365／368／401／402 用 T、6T、4T、2T 四種輸入各試 2–7 次，合計約 1/40，視為不可解。程式仍會按，但對這些投技的防守只能靠蹲／後撤／側移與距離。

**7 幀衝刺投（char 2 的 8137）的解法（2026-09-16）**：蹲 1/14、後撤 0/13、側移 0/6、解投四種輸入約 1/40 都無效；有效的是 **2K**——下段攻擊從第 1 幀起算蹲姿，站立投抓不到，同一場 6 次全部抓空、我們的腳還踢中（`throw_answers.json` 的 "lowkick"）。另外對手站著、距離 75 內時我方先出 T（"T first"，`close_throw.json`）站著 18/25。
